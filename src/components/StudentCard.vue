    ...
    <div class="grid grid-cols-3 w-full mb-10 p-4">
      ...
      <div class="flex gap-x-3 justify-end">
        <div class="flex items-center justify-center cursor-pointer"
             @click="$store.commit('setFullStudent', student)">
          <IconArrowsPointingOut />
        </div>
        <div v-if="currentStudent.length < 2" class="flex items-center justify-center cursor-pointer"
             @click="lockStudentCard">
          <IconPin />
        </div>
        <div v-if="!showHiddenStudents"
...
export default {
  name: "StudentCard",
  components: {
    IconArrowsPointingOut, IconPin, IconEyeSlash, CardFirstView, CardSecondView, CardThirdView, IconEye
  },
  props: {
    student: {
      type: Object,
  ...
  computed: {
    ...mapGetters(['showHiddenStudents', 'currentStudent'])
  },
  methods: {
    ...mapMutations(['setFixedStudent', 'setHiddenStudents', 'setShowHiddenStudents']),
    ...mapActions(['filterStudents']),

    lockStudentCard() {
      this.setFixedStudent(this.student)
      this.filterStudents()
    },
    hideStudent() {
      this.setHiddenStudents(this.student)
      this.filterStudents()
    },
    showStudent() {
      this.setShowHiddenStudents(this.student)
      this.filterStudents()
    },
  }
};
